import os
max = int(input("Enter the number of participant for the program : "))
...
def visitorEntry():
    ...
    file = open("D:\coding-stuff\Assignment Shankar sir\Entry pass with file handling\pass.txt", "a")
    file.write(
        str(participant["num"]) + ", " +
        participant["name"] + ", " +
        participant["place"] + ", " +
...
def Read():
    clear()
    file = open("D:\coding-stuff\Assignment Shankar sir\Entry pass with file handling\pass.txt","r")
    file.seek(0)
    print(file.read())
    file.close()
    input("Press any key to continue")
